...
- Bond / Fear / Hostility relationship model, contextual first impressions, boundaries, romance/intimacy state and NPC dispositions;
...
- player creation / active-persona conversion;
- 24-point PHY/MND/CHA stat buy, starting abilities/spells/inventory;
...
3. In **Character**, create or convert the active persona. PHY + MND + CHA must total 24.
4. In **Settings**, choose a semantic connection if you want a separate sidecar model; otherwise the active connection is used.
5. Keep Prose Guard on **Review** first. Once you like its behavior, switch to Automatic if desired.
6. Roleplay normally. Mechanical information is available in Tracker/Audit, but the narrator only receives a prose-safe outcome handoff.
