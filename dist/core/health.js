...
export function conditionFromActor(actor) {
    const max = Math.max(1, actor.maxHp);
    const hp = Math.max(0, Math.min(max, actor.currentHp));
    if (actor.dead)
        return 'dead';
    if (hp <= 0)
        return actor.nonlethalDefeat ? 'incapacitated' : 'dead';
    const ratio = hp / max;
    if (ratio >= 1)
        return 'healthy';
    if (ratio >= 0.76)
...
export function impairmentForCondition(condition) {
    return { healthy: 0, bruised: -1, wounded: -2, badly_wounded: -4, critical: -6, incapacitated: -999, dead: -999 }[condition];
}
export function damageForOutcome(tier) { return DAMAGE_BY_OUTCOME[tier] ?? 0; }
export function healingForOutcome(tier, magic) {
    return (magic ? MAGIC_HEAL_BY_OUTCOME : NATURAL_HEAL_BY_OUTCOME)[tier] ?? 0;
}
export function safeSceneHealing(magic) { return magic ? 9 : 3; }
export function healingDcForCondition(condition) {
    return { healthy: 13, bruised: 13, wounded: 16, badly_wounded: 19, critical: 19, incapacitated: 19, dead: 19 }[condition];
}
export function healingDcForActor(actor) { return healingDcForCondition(conditionFromActor(actor)); }
export function applyDamage(actor, amount, nonlethal = false, fatal = false) {
    if (actor.dead)
        return;
...
export function applyHeal(actor, amount, naturalTreatment = false) {
    if (actor.dead)
        return false;
    ...
    if (naturalTreatment) {
        ...
        if (actor.naturalTreatmentKey === damageKey)
            return false;
        actor.naturalTreatmentKey = damageKey;
...
export function getActorHealth(state, targetType, target = '') {
    if (targetType === 'user')
        return state.health.user;
    return state.health.npcs[target] ?? null;
}
export function applyHealthEvents(state, events) {
    for (const event of events) {
        const actor = getActorHealth(state, event.targetType, event.target);
        if (!actor)
            continue;
        if (event.kind === 'damage')
            applyDamage(actor, event.amount, event.nonlethal, event.fatal);
        else
            applyHeal(actor, event.amount, event.naturalTreatment === true);
        if (event.targetType === 'npc' && state.npcs[event.target]) {
            const c = conditionFromActor(actor);
            if (c === 'dead')
                state.npcs[event.target].status = 'dead';
            state.npcs[event.target].notes = updateConditionNote(state.npcs[event.target].notes, c);
        }
    }
}
export function applyNaturalRecovery(state, days) {
    const amount = Math.max(0, Math.floor(days)) * 2;
    if (!amount)
        return;
    applyHeal(state.health.user, amount);
    for (const npc of Object.values(state.npcs))
        if (npc.companion && npc.status !== 'dead' && state.health.npcs[npc.name])
            applyHeal(state.health.npcs[npc.name], amount);
}
export function increaseMilestoneHealth(state, companionNames = []) {
...
export function healthSnapshot(health) {
    return {
...
function updateConditionNote(notes, condition) {
