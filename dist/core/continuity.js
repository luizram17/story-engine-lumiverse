...
export function applyContinuitySemantic(state, sem) {
    const notes = [];
    const now = Date.now();
    for (const actor of sem.actors) {
        ...
        if (meaningful && (!wasActive || now >= clock.cooldownUntil)) {
            ...
            if (state.npcs[key]?.romanceStage === 'partner' && now >= clock.partnerMeaningfulUntil)
                clock.partnerMeaningfulUntil = now + PARTNER_MEANINGFUL_COOLDOWN_MS;
        }
        state.continuity.rapportClocks[key] = clock;
    }
    for (const signal of sem.powerActorSignals) {
        if (signal.signal !== 'favor' && signal.signal !== 'grievance' && signal.signal !== 'threat')
            continue;
...
export function boundaryGate(state, sem) {
    ...
    if (br?.present) {
        ...
        if (!valid)
            return { mode: 'none', target: '', type: '', threshold: 1, warnings: 0 };
    ...
    if (!pressure?.present || !pressure.target)
        return { mode: 'none', target: '', type: '', threshold: 1, warnings: 0 };
    ...
    if (immediateOpposition)
        return { mode: 'force', target: pressure.target, type: pressure.kind || 'personal boundary', threshold: 1, warnings: 0 };
...
export function upsertKnowledge(state, input) {
    const subject = clean(input?.subject, 120);
    const fact = clean(input?.fact, 600);
    if (!fact)
        return;
    ...
    if (existing) {
        ...
        if (input?.source)
            existing.source = clean(input.source, 160);
        return;
    }
...
export function archiveDescription(state, input) {
...
export function consumeThreadsForActor(state, actor) {
    for (const list of [state.continuity.latentFavors, state.continuity.latentGrievances])
        for (const t of list)
            if (t.actor.toLowerCase() === actor.toLowerCase() && t.status === 'active')
                t.status = 'consumed';
}
export function resolveBoundary(state, boundaryId) {
    const p = state.continuity.pendingBoundary;
    if (!p.active)
        return;
    if (boundaryId && p.boundaryId !== boundaryId)
        return;
    p.active = false;
}
export function continuitySummary(state) {
...
function updatePendingBoundary(state, sem, notes) {
    const br = sem.boundaryBreak;
    if (br?.present) {
        ...
        if (!valid) {
            notes.push('Rejected boundaryBreak: it did not match the active pending boundary id/target/type.');
            return;
        }
        current.warnings += 1;
        current.lastTurn = state.turn;
    ...
    if (!p?.present || !p.target)
        return;
    const gate = boundaryGate(state, sem);
    const current = state.continuity.pendingBoundary;
...
function refreshBoundCompanion(state) {
    ...
    const bound = state.continuity.boundCompanion;
    if (!best) {
        bound.active = false;
        bound.name = '';
        return;
    }
    if (!bound.active || bound.name !== best.name) {
        state.continuity.boundCompanion = { active: true, name: best.name, sinceTurn: state.turn, lastMeaningfulTurn: state.turn, notes: [] };
    }
    else if (best.bond >= 3 || best.romanceStage !== 'none')
        bound.lastMeaningfulTurn = state.turn;
}
function upsertWorldArc(state, actor, goal, magnitude) {
    let arc = state.continuity.worldArcs.find(x => x.actor.toLowerCase() === actor.toLowerCase() && x.status === 'active');
    if (!arc) {
        arc = { id: `arc_${hash32(`${actor}|${goal}|${state.turn}`).toString(36)}`, actor, goal, stage: 1, pressure: magnitude, lastAdvancedTurn: state.turn, status: 'active' };
        state.continuity.worldArcs.push(arc);
        return;
    }
    arc.goal = goal;
    arc.stage = Math.min(5, arc.stage + 1);
