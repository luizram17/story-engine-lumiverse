...
const defenseByTier = { trivial: 0, cheap: 0, standard: 1, expensive: 2, luxury: 3, elite: 4 };
...
export function addCurrency(list, currency, delta) {
    ...
    if (found)
        found.amount = Math.max(0, found.amount + delta);
    else if (delta > 0)
        next.push({ currency: name, amount: delta });
    return next.filter(x => x.amount > 0);
}
export function applyTransaction(state, tx) {
    if (!tx || tx.kind === 'none' || !state.player)
        return [];
    const notes = [];
...
export function tierFromAmount(amount) {
    let found = 'trivial';
...
export function equipmentDefenseBonus(tier) { return defenseByTier[tier] ?? 0; }
export function isProtectiveEquipment(item) { return protectivePattern.test(item) && !disabledPattern.test(item); }
export function normalizeEquipmentTiers(assignments, owned) {
...
export function resolvePlayerEquipmentDefense(state) {
    if (!state.player)
        return null;
