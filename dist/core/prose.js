 ...
 */
export function buildProseRepairCases(content, findings, limit = 24) {
    ...
    for (const finding of findings) {
        ...
        if (!item) {
            item = { id: `PG${grouped.size + 1}`, start: span.start, end: span.end, sentence: content.slice(span.start, span.end), findings: [] };
            grouped.set(key, item);
        }
        if (!item.findings.some(x => x.category === finding.category && x.phrase.toLowerCase() === finding.phrase.toLowerCase()))
            item.findings.push({ category: finding.category, phrase: finding.phrase });
    }
...
export function proseRepairTool(cases) {
...
export function applyProseRepairPayload(content, cases, payload, extraPhrases = []) {
    ...
    for (const repair of repairs.slice(0, cases.length)) {
        ...
        if (stripStructuredArtifacts(replacement) !== replacement) {
            rejected++;
            continue;
        }
        if (replacement.length < Math.max(4, c.sentence.trim().length * .3) || replacement.length > c.sentence.trim().length * 2.2) {
            rejected++;
            continue;
        }
        ...
        if (collectProseFindings(replacement, extraPhrases).length) {
            rejected++;
            continue;
        }
        if (replacement === c.sentence.trim()) {
            rejected++;
            continue;
        }
    ...
    let text = content;
    for (const r of accepted.sort((a, b) => b.start - a.start))
        text = text.slice(0, r.start) + preserveOuterWhitespace(content.slice(r.start, r.end), r.text) + text.slice(r.end);
...
function sentenceSpans(source) {
    const spans = [];
    const segmenter = typeof Intl !== 'undefined' && typeof Intl.Segmenter === 'function' ? new Intl.Segmenter(undefined, { granularity: 'sentence' }) : null;
    if (segmenter) {
        for (const seg of segmenter.segment(source)) {
            const start = Number(seg.index || 0), end = start + String(seg.segment || '').length;
            if (end > start)
...
function preserveOuterWhitespace(original, replacement) { const lead = original.match(/^\s*/)?.[0] || '', trail = original.match(/\s*$/)?.[0] || ''; return `${lead}${replacement.trim()}${trail}`; }
