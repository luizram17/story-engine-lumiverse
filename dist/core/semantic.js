...
const challengeTypes = ['none', 'social', 'mundane_combat', 'supernatural_combat', 'restraint', 'stealth', 'environment'];
const stats = ['PHY', 'MND', 'CHA', 'NONE'];
const ranks = ['Weak', 'Average', 'Trained', 'Elite', 'Boss'];
const capabilityPools = ['common', 'trained', 'elite', 'boss'];
const weathers = ['clear', 'partly_cloudy', 'cloudy', 'overcast', 'light_rain', 'heavy_rain', 'storm'];
    ...
    return {
        ...
        parameters: {
            ...
            properties: {
                ...
                stealthTargets: { type: 'array', maxItems: 20, items: { type: 'string' } },
                benefitedObservers: { type: 'array', maxItems: 20, items: { type: 'string' } },
                harmedObservers: { type: 'array', maxItems: 20, items: { type: 'string' } },
                npcAwareOfUser: { type: 'array', maxItems: 40, items: { type: 'string' } },
                scene: { type: 'object', additionalProperties: false, properties: { reputationLocation: { type: 'string' }, location: { type: 'string' }, area: { type: 'string' }, indoors: { type: 'boolean' }, timeAdvance: { type: 'integer', minimum: 0, maximum: 4 }, timeOfDay: { type: 'string', enum: ['morning', 'afternoon', 'evening', 'night'] }, weather: { type: 'string', enum: ['', ...weathers] }, presentNpcs: { type: 'array', maxItems: 40, items: { type: 'string' } }, publicWitnesses: { type: 'boolean' }, danger: { type: 'string', enum: ['calm', 'active', 'crisis'] } }, required: ['presentNpcs', 'publicWitnesses', 'danger'] },
...
export function buildSemanticPrompt(input) {
    return [
...
RELATIONSHIP EVIDENCE: actor.relation=benefited is reserved for a concrete material/stakes benefit such as rescue, meaningful protection, resources, restored autonomy, prevention of real harm/loss, major standing improvement, or explicit goal advancement. Politeness, flirting, casual help, ordinary cooperation, and choosing not to harm are not benefited. For ordinary positive familiarity, report slowBondEvidence instead. Set only categories actually evidenced this scene: respectfulContact, cooperation, comfortInProximity, boundaryRespect, sharedRoutine, playfulness, teamwork, personalAttention. slowBondBlockers lists fresh explicit closeness blockers such as coercion, betrayal, humiliation, unwanted intimacy pressure, boundary violation, unresolved user-caused harm, exploitation, active fear/hostility, or trapped/dependent/powerless circumstances.

NPC AWARENESS / OBSERVERS: npcAwareOfUser lists NPCs who individually notice, address, react to, or interact with the user this turn. benefitedObservers/harmedObservers are third parties whose concrete stakes materially improve/worsen, not mere witnesses. stealthTargets is the exact established living detector list for an explicit stealth attempt.

STANDING: standingInfluence is about recognized external standing, NOT affection: title, authority, reputation, demonstrated power, backing, lineage, or affiliation that this NPC actually knows and recognizes. Use constrained only when it limits what the NPC openly dares to express/do; aware when it changes etiquette/caution without constraining them; otherwise none. standingBasis is concise evidence. Standing never directly changes Bond/Fear/Hostility. romanceStyle is a stable initiative style: nervous for clearly shy/guarded/reserved, flirt for clearly bold/playful/direct, auto if unclear.
...
export function normalizeSemanticLedger(value) {
    const src = object(value);
    const scene = object(src.scene);
    const bp = object(src.boundaryPressure);
    const restraint = object(src.restraintControl);
    const boundaryBreak = object(src.boundaryBreak);
    const claim = object(src.claimCheck);
    const tx = object(src.transaction);
    const loot = object(src.loot);
    ...
    const actions = (Array.isArray(src.actions) ? src.actions : []).slice(0, MAX_ACTIONS).map(normalizeAction).filter(a => {
        if (budget <= 0)
            return false;
        a.actionLength = Math.max(1, Math.min(a.actionLength, budget));
    ...
    return {
        ...
        stealthTargets: cleanNameList(src.stealthTargets, 20),
        benefitedObservers: cleanNameList(src.benefitedObservers, 20),
        harmedObservers: cleanNameList(src.harmedObservers, 20),
        npcAwareOfUser: cleanNameList(src.npcAwareOfUser, 40),
        scene: {
            reputationLocation: text(scene.reputationLocation) || undefined, location: text(scene.location) || undefined, area: text(scene.area) || undefined, indoors: typeof scene.indoors === 'boolean' ? scene.indoors : undefined,
            timeAdvance: clampInt(scene.timeAdvance, 0, 4, 0),
            timeOfDay: ['morning', 'afternoon', 'evening', 'night'].includes(String(scene.timeOfDay)) ? scene.timeOfDay : undefined,
            weather: weathers.includes(String(scene.weather)) ? scene.weather : '', presentNpcs: Array.isArray(scene.presentNpcs) ? [...new Set(scene.presentNpcs.map((x) => text(x).slice(0, 120)).filter(Boolean))].slice(0, 40) : undefined, publicWitnesses: bool(scene.publicWitnesses, false),
            danger: ['calm', 'active', 'crisis'].includes(String(scene.danger)) ? scene.danger : 'calm',
        },
        ...
        loot: Object.keys(loot).length ? { present: bool(loot.present, false), target: text(loot.target), targetKind: ['humanoid', 'monster', 'container', 'other'].includes(String(loot.targetKind)) ? loot.targetKind : 'other', rank: ranks.includes(String(loot.rank)) ? loot.rank : 'Average' } : undefined,
        memoryFacts: (Array.isArray(src.memoryFacts) ? src.memoryFacts : []).slice(0, 20).map((x) => { const o = object(x); return { fact: text(o.fact).slice(0, 600), scope: ['scene', 'location', 'world', 'user', 'npc'].includes(String(o.scope)) ? o.scope : 'world', subject: text(o.subject) || undefined, salience: clampInt(o.salience, 1, 5, 2) }; }).filter((x) => x.fact),
        namesNeeded: (Array.isArray(src.namesNeeded) ? src.namesNeeded : []).slice(0, 12).map((x) => { const o = object(x); return { kind: ['npc', 'place', 'organization'].includes(String(o.kind)) ? o.kind : 'npc', hint: text(o.hint).slice(0, 160) }; }),
        powerActorSignals: (Array.isArray(src.powerActorSignals) ? src.powerActorSignals : []).slice(0, 12).map((x) => { const o = object(x); return { actor: text(o.actor), signal: ['notice', 'favor', 'grievance', 'threat', 'opportunity'].includes(String(o.signal)) ? o.signal : 'notice', magnitude: clampInt(o.magnitude, 1, 3, 1) }; }).filter((x) => x.actor),
    };
}
export function fallbackSemanticLedger(userMessage) {
    ...
    const challengeType = harmful ? 'mundane_combat' : social ? 'social' : stealth ? 'stealth' : 'none';
    const action = { label: userMessage.slice(0, 160), kind: harmful ? 'attack' : social ? 'social' : stealth ? 'stealth' : heal ? 'heal' : 'other', target: '', challengeType, rollNeeded: challengeType !== 'none', stat: challengeType === 'social' ? 'CHA' : 'PHY', difficulty: 3, actionLength: 1, harmful, harmMode: harmful ? (lower.includes('kill') ? 'lethal' : 'nonlethal') : 'none', supernatural: false, healingMagic: false, socialTactic: social ? (lower.includes('intimid') || lower.includes('threat') ? 'intimidate' : lower.includes('lie') || lower.includes('deceiv') ? 'bluff' : 'diplomacy') : 'none', socialGoal: social ? userMessage.slice(0, 100) : '' };
    return { summary: userMessage.slice(0, 300), actions: [action], actors: [], explicitIntimidationOrCoercion: /\b(threaten|intimidat|coerc)\b/i.test(userMessage), intimacyAdvanceExplicit: /\b(kiss|date|romance|sex|intimate)\b/i.test(userMessage), stealthTargets: [], benefitedObservers: [], harmedObservers: [], npcAwareOfUser: [], scene: { publicWitnesses: false, danger: harmful ? 'active' : 'calm' }, memoryFacts: [], namesNeeded: [], powerActorSignals: [] };
}
function normalizeAction(value) {
    const o = object(value);
    const challenge = challengeTypes.includes(String(o.challengeType)) ? o.challengeType : 'none';
    const kind = ['attack', 'social', 'stealth', 'environment', 'restraint', 'heal', 'loot', 'transaction', 'other'].includes(String(o.kind)) ? o.kind : 'other';
    return { label: text(o.label).slice(0, 200), kind, target: text(o.target).slice(0, 120), challengeType: challenge, rollNeeded: bool(o.rollNeeded, false), stat: stats.includes(String(o.stat)) ? o.stat : 'NONE', targetStat: stats.includes(String(o.targetStat)) ? o.targetStat : 'NONE', difficulty: clampInt(o.difficulty, 1, 5, 3), actionLength: clampInt(o.actionLength, 1, 3, 1), harmful: bool(o.harmful, false), harmMode: ['none', 'lethal', 'nonlethal', 'restraint_control'].includes(String(o.harmMode)) ? o.harmMode : (challenge === 'restraint' ? 'restraint_control' : 'none'), supernatural: bool(o.supernatural, false), healingMagic: bool(o.healingMagic, false), abilityUse: text(o.abilityUse) || undefined, itemUse: text(o.itemUse) || undefined, socialTactic: ['diplomacy', 'bluff', 'intimidate', 'none'].includes(String(o.socialTactic)) ? o.socialTactic : 'none', socialGoal: text(o.socialGoal).slice(0, 160) || undefined };
}
function normalizeActor(value) {
    const o = object(value);
    const slow = object(o.slowBondEvidence);
    const slowKeys = ['respectfulContact', 'cooperation', 'comfortInProximity', 'boundaryRespect', 'sharedRoutine', 'playfulness', 'teamwork', 'personalAttention'];
    const slowEvidence = {};
    for (const k of slowKeys)
        if (k in slow)
            slowEvidence[k] = bool(slow[k], false);
    return {
        name: text(o.name).slice(0, 120), role: text(o.role).slice(0, 120) || 'NPC',
        rank: ranks.includes(String(o.rank)) ? o.rank : 'Average',
        capabilityPool: capabilityPools.includes(String(o.capabilityPool)) ? o.capabilityPool : undefined,
        mainStat: ['PHY', 'MND', 'CHA', 'Balanced'].includes(String(o.mainStat)) ? o.mainStat : undefined,
        relation: ['direct', 'opposed', 'benefited', 'harmed', 'observer', 'neutral'].includes(String(o.relation)) ? o.relation : 'neutral',
        powerActor: bool(o.powerActor, false), companion: bool(o.companion, false),
        initialBond: o.initialBond == null ? undefined : clampInt(o.initialBond, 0, 4, 0),
        initialFear: o.initialFear == null ? undefined : clampInt(o.initialFear, 0, 4, 0),
        initialHostility: o.initialHostility == null ? undefined : clampInt(o.initialHostility, 0, 4, 0),
        initialRomanceStage: ['none', 'interest', 'dating', 'partner'].includes(String(o.initialRomanceStage)) ? o.initialRomanceStage : undefined,
        initialIntimacy: o.initialIntimacy == null ? undefined : clampInt(o.initialIntimacy, 0, 4, 0),
        personalityArchetype: text(o.personalityArchetype).slice(0, 80) || undefined,
        personalitySummary: text(o.personalitySummary).slice(0, 500) || undefined,
        initialNotes: Array.isArray(o.initialNotes) ? o.initialNotes.map((x) => text(x)).filter(Boolean).slice(0, 8) : undefined,
        relationshipContext: text(o.relationshipContext).slice(0, 300) || undefined,
        initialRelationshipDescriptors: Array.isArray(o.initialRelationshipDescriptors) ? o.initialRelationshipDescriptors.map((x) => text(x).slice(0, 100)).filter(Boolean).slice(0, 12) : undefined,
        romanceStyle: ['auto', 'nervous', 'flirt'].includes(String(o.romanceStyle)) ? o.romanceStyle : undefined,
        standingInfluence: ['none', 'aware', 'constrained'].includes(String(o.standingInfluence)) ? o.standingInfluence : undefined,
        standingBasis: text(o.standingBasis).slice(0, 240) || undefined,
        slowBondEvidence: Object.keys(slowEvidence).length ? slowEvidence : undefined,
        slowBondBlockers: Array.isArray(o.slowBondBlockers) ? o.slowBondBlockers.map((x) => text(x).slice(0, 120)).filter(Boolean).slice(0, 12) : undefined,
        evidence: text(o.evidence).slice(0, 240) || undefined,
    };
}
function cleanNameList(value, max) { return [...new Set((Array.isArray(value) ? value : []).map((x) => text(x).slice(0, 120)).filter(Boolean))].slice(0, max); }
