...
export function applyWorldSemantic(state, sem, seed) {
    const notes = [];
    if (sem.scene.reputationLocation)
        state.world.reputationLocation = sem.scene.reputationLocation;
    if (sem.scene.location && sem.scene.location !== state.world.location) {
        state.world.location = sem.scene.location;
        state.world.positionEstablished = true;
        notes.push(`Location: ${sem.scene.location}`);
        if (!state.world.reputationLocation)
            state.world.reputationLocation = sem.scene.location;
    }
    if (sem.scene.area) {
    ...
    if (sem.scene.timeOfDay) {
        state.world.time = sem.scene.timeOfDay;
        state.world.timeEstablished = true;
    }
    if (sem.scene.weather) {
        state.world.weather = sem.scene.weather;
        state.world.weatherEstablished = true;
        state.world.weatherRemainingSlots = Math.max(1, state.world.weatherRemainingSlots || 2);
    }
...
export function worldSummary(state, sem) {
...
export function observablePlanEvidence(state, sem) {
    ...
    for (const plan of state.world.plans.filter(p => p.status === 'due' || p.status === 'pending')) {
        for (const evidence of plan.evidence || []) {
            ...
            if (open)
                result.push({ planId: plan.id, id: evidence.id, topic: evidence.topic, text: evidence.text, route: evidence.route });
            if (result.length >= 4)
                return result;
        }
    }
