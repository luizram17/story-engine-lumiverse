...
export function extractOocCommands(input) {
    const out = [];
    const re = /\(\(([\s\S]*?)\)\)/g;
    let match;
...
export function stripOocCommands(input) {
...
export function normalizeMutationBatch(value) {
    const o = object(value);
    const operations = [];
    for (const raw of Array.isArray(o.operations) ? o.operations : []) {
        const r = object(raw);
        const op = String(r.op || '');
        if (!['set', 'increment', 'append', 'remove_value', 'delete', 'merge'].includes(op))
            continue;
        const path = (Array.isArray(r.path) ? r.path : []).filter(x => typeof x === 'string' || Number.isInteger(x)).slice(0, 8);
        if (!path.length)
            continue;
        const root = String(path[0]);
        if (!ALLOWED_ROOTS.has(root) || BLOCKED_ROOTS.has(root))
            continue;
        operations.push({ op: op, path, value: r.value });
    }
    return { summary: text(o.summary).slice(0, 600), operations: operations.slice(0, 80) };
}
export function commandAssistantPrompt(commands, stateContext, recentContext) {
    return [
...
- Add/remove player wounds, conditions, tasks or commitments through their corresponding player arrays.
- NPCs can carry inventory, gear, currency, wounds and conditions. Their relationship can also carry romanceStyle, standingInfluence/standingBasis and slowBondEvidence when a command explicitly establishes those facts.
- Set who is physically present right now: ["world","presentNpcs"] with the complete list of tracker names/stable labels.
- Add a world fact: append to ["world","facts"] with a complete fact object. World plans may be created/retconned under ["world","plans"] when the OOC command explicitly establishes a schedule, NPC/faction objective, deadline or off-screen plan; descriptive identity/history/connections may be edited under ["continuity","descriptiveArchive"].
...
export function bootstrapAssistantPrompt(transcriptChunk, stateContext, personaContext, chunkIndex, chunkCount) {
    return [
...
export function applyMutationBatch(input, batch) {
    let draft = JSON.parse(JSON.stringify(input));
    const notes = [];
    for (const mutation of batch.operations) {
        try {
            applyOne(draft, mutation);
            notes.push(`${mutation.op} ${formatPath(mutation.path)}`);
        }
        catch (err) {
            notes.push(`Skipped ${formatPath(mutation.path)}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    const state = normalizeState(draft);
    for (const npc of Object.values(state.npcs)) {
        ensureNpc(state, npc.name, npc.rank, npc.role, `mutation|${state.turn}`, 'Balanced');
        enforceRelationshipInvariants(npc);
    }
    return { state, notes };
}
function applyOne(root, m) {
    const path = m.path;
    if (!path.length)
        throw new Error('empty path');
    const rootName = String(path[0]);
    if (!ALLOWED_ROOTS.has(rootName) || BLOCKED_ROOTS.has(rootName))
        throw new Error('path root not allowed');
    if (path.length === 1) {
        if (m.op === 'delete')
            throw new Error('cannot delete a root namespace');
        if (m.op === 'set') {
            root[rootName] = clone(m.value);
            return;
        }
        if (m.op === 'merge') {
            root[rootName] = deepMerge(root[rootName], m.value);
            return;
        }
        if (m.op === 'increment') {
            root[rootName] = Number(root[rootName] || 0) + Number(m.value || 0);
            return;
        }
        if (m.op === 'append') {
            if (!Array.isArray(root[rootName]))
                root[rootName] = [];
            root[rootName].push(clone(m.value));
            return;
        }
        if (m.op === 'remove_value') {
            if (Array.isArray(root[rootName]))
                root[rootName] = root[rootName].filter((x) => !sameValue(x, m.value));
            return;
        }
    }
    ...
    if (m.op === 'increment') {
        parent[key] = Number(parent[key] || 0) + Number(m.value || 0);
        return;
    }
    if (m.op === 'append') {
        if (!Array.isArray(parent[key]))
            parent[key] = [];
        parent[key].push(clone(m.value));
        return;
    }
    if (m.op === 'remove_value') {
        if (Array.isArray(parent[key]))
            parent[key] = parent[key].filter((x) => !sameValue(x, m.value));
        return;
    }
    if (m.op === 'merge') {
        parent[key] = deepMerge(parent[key], m.value);
        return;
    }
}
function getParent(root, path, create) {
    let cur = root;
    for (let i = 0; i < path.length - 1; i++) {
        ...
        if (typeof cur[key] !== 'object')
            throw new Error(`non-container ${String(key)}`);
        cur = cur[key];
    }
    return cur;
}
...
function sameValue(a, b) { try {
    return JSON.stringify(a) === JSON.stringify(b);
}
catch {
    return a === b;
} }
function formatPath(path) { return path.map(x => typeof x === 'number' ? `[${x}]` : String(x)).join('.'); }
