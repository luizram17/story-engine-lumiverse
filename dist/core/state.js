...
export function normalizeState(value) {
    ...
    return {
        ...
        world: {
            ...
            time: ['morning', 'afternoon', 'evening', 'night'].includes(String(world.time)) ? world.time : 'morning',
            weather: ['clear', 'partly_cloudy', 'cloudy', 'overcast', 'light_rain', 'heavy_rain', 'storm'].includes(String(world.weather)) ? world.weather : 'clear',
            facts: Array.isArray(world.facts) ? world.facts.slice(-MAX_MEMORY_FACTS).filter(Boolean) : [],
            plans: Array.isArray(world.plans) ? world.plans.slice(-80).filter(Boolean) : [],
        },
        ...
        continuity: {
            ...
            descriptiveArchive: Array.isArray(continuity.descriptiveArchive) ? continuity.descriptiveArchive.slice(-240) : [],
...
function normalizePlayer(value) {
    ...
    const player = {
        ...
        currency: Array.isArray(src.currency) ? src.currency : [], gear: stringList(src.gear), anchors: stringList(src.anchors), concept: text(src.concept), backstory: text(src.backstory),
    };
...
function normalizeRapportClocks(value) {
    ...
    for (const [name, raw] of Object.entries(object(value))) {
        const c = object(raw);
        out[name] = { lastInteractionAt: Math.max(0, num(c.lastInteractionAt, 0)), lastMeaningfulAt: Math.max(0, num(c.lastMeaningfulAt, 0)), cooldownUntil: Math.max(0, num(c.cooldownUntil, 0)), partnerMeaningfulUntil: Math.max(0, num(c.partnerMeaningfulUntil, 0)) };
    }
    return out;
}
