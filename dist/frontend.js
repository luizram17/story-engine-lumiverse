...
export function setup(ctx) {
  ...
  `);
    ...
    let widget = null;
    try {
        widget = ctx.ui.createFloatWidget({ width: 300, height: 390, initialPosition: { x: 18, y: 120 }, snapToEdge: true, tooltip: 'Story Engine player HUD', chromeless: true });
        widget.root.innerHTML = '<div class="se-widget-panel"><div class="se-widget-head"><div class="se-widget-title"><b>Story Engine</b><span class="se-muted">Loading player HUD…</span></div></div></div>';
    }
    catch { }
    ...
    function wire(shell) {
        ...
        const updatePointBuy = () => { const f = shell.querySelector('#se-character'); if (!f)
            ...
            info.classList.toggle('is-ok', valid);
            info.classList.toggle('is-bad', !valid);
        } const btn = shell.querySelector('[data-create-player]'); if (btn)
            btn.disabled = !valid || busy === 'player'; };
        shell.querySelectorAll('[data-stat-buy]').forEach(el => el.addEventListener('input', updatePointBuy));
        shell.querySelector('[data-create-player]')?.addEventListener('click', () => { const f = shell.querySelector('#se-character'); const fd = new FormData(f); const stats = { PHY: Number(fd.get('PHY')), MND: Number(fd.get('MND')), CHA: Number(fd.get('CHA')) }; const total = stats.PHY + stats.MND + stats.CHA; if (total !== 15 || Object.values(stats).some(v => !Number.isInteger(v) || v < 1 || v > 9)) {
            alert(`Stats must use exactly 15 points, with each stat from 1 to 9. Current total: ${total}.`);
            return;
    ...
    function updateWidget() {
        ...
        if (!dashboard.chatId) {
            ...
            widget.root.querySelector('[data-widget-open]')?.addEventListener('click', () => tab.activate());
            return;
        }
        if (!p) {
            widget.root.innerHTML = '<div class="se-widget-panel"><div class="se-widget-head"><div class="se-widget-title"><b>Story Engine</b><span class="se-muted">Chat attached</span></div><button class="se-widget-open" data-widget-open>Configure</button></div><div class="se-widget-scroll"><div class="se-widget-empty">No player sheet yet. Create one or convert the active persona in the Character tab.</div></div></div>';
            widget.root.querySelector('[data-widget-open]')?.addEventListener('click', () => { activeView = 'character'; tab.activate(); render(); });
            return;
        }
        const money = (p.currency || []).map((x) => `${x.amount} ${x.currency}`).join(' · ') || '—';
...
function renderTracker(s, p) {
  ...
  <section class="se-section"><header>World memory</header><div class="se-body"><div class="se-list">${facts.length ? facts.map((f) => `<div class="se-card"><span class="se-chip">${esc(f.scope)}</span> ${esc(f.fact)}</div>`).join('') : '<div class="se-muted">No durable facts archived yet.</div>'}</div>${archive.length ? `<div class="se-list">${archive.map((a) => `<div class="se-card"><span class="se-chip">${esc(a.kind)}</span> <b>${esc(a.promotedName || a.label)}</b><div class="se-muted">${esc(a.description)}${a.affiliation ? ` · ${esc(a.affiliation)}` : ''}${a.lastKnownLocation ? ` · last seen ${esc(a.lastKnownLocation)}` : ''}</div>${a.history?.length ? `<div class="se-muted"><b>History:</b> ${esc(a.history.slice(-3).join(' · '))}</div>` : ''}${a.connections?.length ? `<div class="se-muted"><b>Connections:</b> ${esc(a.connections.join(' · '))}</div>` : ''}</div>`).join('')}</div>` : ''}${plans.length ? `<div class="se-list">${plans.map((plan) => `<div class="se-card"><span class="se-chip">${esc(plan.status)}</span><span class="se-chip">${esc(plan.kind || 'plan')}</span> <b>${esc(plan.actor)}</b> · ${esc(plan.intent)}<div class="se-muted">Due turn ${esc(plan.dueTurn)}${plan.evidence?.some((e) => e.discovered) ? ` · discovered leads ${esc(plan.evidence.filter((e) => e.discovered).length)}` : ''}</div></div>`).join('')}</div>` : ''}</div></section>`;
}
function renderPlayerResources(s, p) {
    if (!p)
        return `<section class="se-section"><header>Player resources</header><div class="se-body"><div class="se-muted">Create or convert a player character to populate inventory, money, equipment and abilities.</div></div></section>`;
...
function widgetPresentNpcs(s) {
    const names = Array.isArray(s.world?.presentNpcs) ? s.world.presentNpcs : [];
    if (!names.length)
        return `<div class="se-widget-section"><span class="se-widget-label">NPCs here</span><div class="se-widget-empty">None established in the current scene</div></div>`;
    const rows = names.map((name) => { const n = s.npcs?.[name]; const h = s.health?.npcs?.[name]; const condition = h ? visibleNpcCondition(h) : ''; const rel = n?.relationshipDescriptors?.[0] || n?.role || ''; const tags = [rel, n?.companion ? 'companion' : '', condition].filter(Boolean); return `<div class="se-widget-line"><b>${esc(name)}</b>${tags.length ? `<div class="se-muted">${esc(tags.join(' · '))}</div>` : ''}</div>`; }).join('');
...
function renderSettings(s, connections) {
    const conn = (name, selected, inherit = 'Auto · default usable profile') => `<select name="${attr(name)}"><option value="">${esc(inherit)}</option>${connectionOptions(connections, selected)}</select>`;
