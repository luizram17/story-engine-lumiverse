...
export function setup(ctx) {
  ...
  `);
    ...
    const unsubChatChanged = ctx.events.on('CHAT_CHANGED', (payload) => { chatHint = String(payload?.chatId || chatHint || ''); request('get_dashboard', { chatId: chatHint }); });
...
function renderSettings(s, connections) {
    const conn = (name, selected, inherit = 'Current active connection') => `<select name="${attr(name)}"><option value="">${esc(inherit)}</option>${connectionOptions(connections, selected)}</select>`;
  ...
  <div class="se-grid"><div class="se-field"><label>Prose Guard mode</label><select name="proseGuardMode">${options(['off', 'review', 'automatic'], s.proseGuardMode || 'review')}</select></div><div class="se-field"><label>Prose Guard connection</label>${conn('proseGuardConnectionId', s.proseGuardConnectionId, 'Use semantic/current connection')}</div></div>
...
function renderBootstrapStatus(s, busy) { const b = s.bootstrap || { status: 'none', sourceMessageCount: 0 }; if (b.status === 'importing' || busy === 'history')
    return `<section class="se-section"><header>Existing chat attachment</header><div class="se-body"><div class="se-warn">Importing the existing conversation history… ${esc(b.sourceMessageCount || '')} stored message(s) detected.</div></div></section>`; if (b.status === 'ready')
    return `<section class="se-section"><header>Existing chat attachment</header><div class="se-body"><div class="se-card"><b>History attached</b><div class="se-muted">${esc(b.sourceMessageCount || 0)} canonical messages were read to reconstruct Story Engine state.</div></div><button class="se-btn" data-import-history>Re-import entire history</button></div></section>`; if (b.status === 'failed')
    return `<section class="se-section"><header>Existing chat attachment</header><div class="se-body"><div class="se-warn"><b>History import failed.</b><div class="se-muted">${esc(b.error || 'Unknown error')}</div></div><button class="se-btn" data-import-history>Retry history import</button></div></section>`; return `<section class="se-section"><header>Existing chat attachment</header><div class="se-body"><div class="se-muted">Story Engine can attach to a conversation that already has messages and reconstruct its state from the full saved history.</div><button class="se-btn" data-import-history>Import existing history now</button></div></section>`; }
