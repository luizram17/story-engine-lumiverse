...
export function setup(ctx:Ctx){
  ...
  function render(){
    ...
    if(activeView==='overview')main.innerHTML=renderOverview(s,settings,p,busy);
...
function renderBootstrapStatus(s:any,busy:string){const b=s.bootstrap||{status:'none',sourceMessageCount:0};if(b.status==='importing'||busy==='history')return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-warn">Importing the existing conversation history… ${esc(b.sourceMessageCount||'')} stored message(s) detected.</div></div></section>`;if(b.status==='ready'){const n=Number(b.sourceMessageCount||0);return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-card"><b>Story Engine attached</b><div class="se-muted">${n<3?`${esc(n)} prior message(s) found; no history reconstruction was necessary.`:`${esc(n)} canonical messages were read to reconstruct Story Engine state.`}</div></div>${n>=3?'<button class="se-btn" data-import-history>Re-import entire history</button>':''}</div></section>`;}if(b.status==='failed')return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-warn"><b>Attached, but history import needs attention.</b><div class="se-muted">${esc(b.error||'Unknown error')}</div></div><button class="se-btn" data-import-history>Retry history import</button></div></section>`;return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-muted">Story Engine attaches automatically. If this conversation already contains RP history, it can reconstruct its state from the saved messages.</div><button class="se-btn" data-import-history>Import existing history now</button></div></section>`;}
