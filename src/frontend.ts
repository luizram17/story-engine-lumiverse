...
function renderBootstrapStatus(s:any,busy:string,dashboard:any){const b=s.bootstrap||{status:'none',sourceMessageCount:0};const live=Number.isFinite(Number(dashboard?.liveMessageCount))?Number(dashboard.liveMessageCount):null;const observed=live??Number(b.sourceMessageCount||0);if(b.status==='importing'||busy==='history')return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-warn">Importing the existing conversation history… ${esc(observed)} canonical message(s) detected.</div></div></section>`;if(b.status==='ready'){const n=Number(b.sourceMessageCount||0);const stale=live!=null&&live>n&&n<3;return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-card"><b>Story Engine attached</b><div class="se-muted">${stale?`The host currently reports ${esc(live)} canonical messages; the previous ${esc(n)}-message early attach is stale and will be reconstructed.`:n<3?`${esc(n)} canonical message(s) found at attachment time; no reconstruction was necessary.`:`${esc(n)} canonical messages were read to reconstruct Story Engine state.`}</div></div>${n>=3||stale?'<button class="se-btn" data-import-history>Re-import entire history</button>':''}</div></section>`;}if(b.status==='failed')return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-warn"><b>Attached, but history import needs attention.</b><div class="se-muted">${esc(b.error||'Unknown error')}</div></div><button class="se-btn" data-import-history>Retry history import</button></div></section>`;return `<section class="se-section"><header>Chat attachment</header><div class="se-body"><div class="se-muted">Story Engine attaches automatically. ${live!=null?`Lumiverse currently reports ${esc(live)} canonical message(s) in this chat. `:''}If this conversation already contains RP history, it will reconstruct its state from those saved messages.</div><button class="se-btn" data-import-history>Import existing history now</button></div></section>`;}
