...
export function setup(ctx:Ctx){
  ...
  function updateWidget(){
    ...
    const scene=[w.location,w.area].filter(Boolean).join(' · ')||'Unknown location';
    ...
    widget.root.innerHTML=`<div class="se-widget-panel">
      ...
      <div class="se-widget-scroll">
        ...
        <div class="se-widget-section"><span class="se-widget-label">Current scene</span><div class="se-widget-line">${esc(scene)}</div><div class="se-muted">day ${esc(w.dayIndex||1)} · ${esc(w.time||'')} · ${esc(w.weather||'')}</div></div>
      </div></div>`;
...
function renderTracker(s:any,p:any){
  ...
  <section class="se-section"><header>World memory</header><div class="se-body"><div class="se-list">${facts.length?facts.map((f:any)=>`<div class="se-card"><span class="se-chip">${esc(f.scope)}</span> ${esc(f.fact)}</div>`).join(''):'<div class="se-muted">No durable facts archived yet.</div>'}</div></div></section>`;
}
...
function renderPlayerResources(s:any,p:any){
  ...
  return `<section class="se-section"><header>Player resources</header><div class="se-body"><div class="se-grid-3"><div class="se-kpi"><span class="se-muted">Health</span><b>${esc(h?`${h.currentHp} / ${h.maxHp}`:'—')}</b></div><div class="se-kpi"><span class="se-muted">Level / XP</span><b>Lv ${esc(pr.level||1)} · ${esc(pr.xp||0)} XP</b><span class="se-muted">${esc(pr.pendingMilestones||0)} milestone(s) pending</span></div><div class="se-kpi"><span class="se-muted">Money</span><b>${esc(money)}</b>${quote?`<span class="se-muted">Quoted: ${esc(quote)}</span>`:''}</div></div><div class="se-grid"><div class="se-card"><b>Inventory</b><div class="se-muted">${esc((p.inventory||[]).join(' · ')||'—')}</div></div><div class="se-card"><b>Equipment</b><div class="se-muted">${esc((p.gear||[]).join(' · ')||'—')}</div></div><div class="se-card"><b>Abilities</b><div class="se-muted">${esc((p.abilities||[]).join(' · ')||'—')}</div></div><div class="se-card"><b>Spells</b><div class="se-muted">${esc((p.spells||[]).join(' · ')||'—')}</div></div>${p.naturalWeapons?.length?`<div class="se-card"><b>Natural weapons</b><div class="se-muted">${esc(p.naturalWeapons.join(' · '))}</div></div>`:''}${p.anchors?.length?`<div class="se-card"><b>Character anchors</b><div class="se-muted">${esc(p.anchors.join(' · '))}</div></div>`:''}</div></div></section>`;
}
