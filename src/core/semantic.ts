import type { CapabilityPool, ChallengeType, Rank, SemanticAction, SemanticActor, SemanticLedger, SlowBondKey, StatKey, Weather } from '../shared/types.js';
...
export function normalizeSemanticLedger(value:unknown):SemanticLedger {
  ...
  return {
    ...
    scene:{
      ...
      timeOfDay:['morning','afternoon','evening','night'].includes(String(scene.timeOfDay))?scene.timeOfDay as any:undefined,
...
function normalizeActor(value:unknown):SemanticActor{
  const o=object(value);
  const slow=object(o.slowBondEvidence);
  const slowKeys:SlowBondKey[]=['respectfulContact','cooperation','comfortInProximity','boundaryRespect','sharedRoutine','playfulness','teamwork','personalAttention'];
  const slowEvidence:Partial<Record<SlowBondKey,boolean>>={};
  for(const k of slowKeys)if(k in slow)slowEvidence[k]=bool(slow[k],false);
  return{
    name:text(o.name).slice(0,120),role:text(o.role).slice(0,120)||'NPC',
    rank:ranks.includes(String(o.rank) as Rank)?o.rank as Rank:'Average',
    capabilityPool:capabilityPools.includes(String(o.capabilityPool) as CapabilityPool)?o.capabilityPool as CapabilityPool:undefined,
    mainStat:['PHY','MND','CHA','Balanced'].includes(String(o.mainStat))?o.mainStat as any:undefined,
    ...
    initialRomanceStage:['none','interest','dating','partner'].includes(String(o.initialRomanceStage))?o.initialRomanceStage as any:undefined,
    ...
    initialNotes:Array.isArray(o.initialNotes)?o.initialNotes.map((x:any)=>text(x)).filter(Boolean).slice(0,8):undefined,
    relationshipContext:text(o.relationshipContext).slice(0,300)||undefined,
    initialRelationshipDescriptors:Array.isArray(o.initialRelationshipDescriptors)?o.initialRelationshipDescriptors.map((x:any)=>text(x).slice(0,100)).filter(Boolean).slice(0,12):undefined,
    romanceStyle:['auto','nervous','flirt'].includes(String(o.romanceStyle))?o.romanceStyle as any:undefined,
    standingInfluence:['none','aware','constrained'].includes(String(o.standingInfluence))?o.standingInfluence as any:undefined,
    ...
    slowBondBlockers:Array.isArray(o.slowBondBlockers)?o.slowBondBlockers.map((x:any)=>text(x).slice(0,120)).filter(Boolean).slice(0,12):undefined,
    evidence:text(o.evidence).slice(0,240)||undefined,
  };
}
function cleanNameList(value:unknown,max:number):string[]{return [...new Set((Array.isArray(value)?value:[]).map((x:any)=>text(x).slice(0,120)).filter(Boolean))].slice(0,max);}
