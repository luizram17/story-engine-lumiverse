...
export function normalizeSettings(value: unknown): StorySettings {
  ...
  return {
    ...
    trackerWidgetBackgroundOpacity: clampNum(src.trackerWidgetBackgroundOpacity, 0.4, 1, DEFAULT_SETTINGS.trackerWidgetBackgroundOpacity),
...
export function normalizeState(value: unknown): StoryState {
  ...
  for (const [name,npc] of Object.entries(npcs)) {
    ...
    if(npc.companion&&npcHealth[name]!.maxHp<10){
      const actor=npcHealth[name]!;const delta=10-actor.maxHp;actor.maxHp=10;actor.currentHp=Math.min(actor.maxHp,actor.currentHp+delta);
    }
  }
  ...
  return {
    ...
    world: (()=>{
      const location=text(world.location); const area=text(world.area); const indoors=bool(world.indoors,false);
      const dayIndex=Math.max(1,Math.floor(num(world.dayIndex,1)));
      const time=['morning','afternoon','evening','night'].includes(String(world.time))?world.time as any:'morning';
      const weatherValue=['clear','partly_cloudy','cloudy','overcast','light_rain','heavy_rain','storm'].includes(String(world.weather))?world.weather as any:'clear';
      ...
      return {reputationLocation:text(world.reputationLocation)||location,location,area,indoors,positionEstablished,dayIndex,time,timeEstablished,weather:weatherValue,weatherRemainingSlots,weatherEstablished,presentNpcs:[...new Set((Array.isArray(world.presentNpcs)?stringList(world.presentNpcs):legacyPresence).map(x=>x.slice(0,120)))].slice(0,40),facts:Array.isArray(world.facts)?world.facts.slice(-MAX_MEMORY_FACTS).filter(Boolean) as any:[],plans:normalizeWorldPlans(world.plans)};
    })(),
    ...
    bootstrap: {
      ...
      startedAt: num(bootstrap.startedAt,0)||undefined,
      updatedAt: num(bootstrap.updatedAt,0)||undefined,
      stage: text(bootstrap.stage)||undefined,
      completedChunks: Math.max(0,Math.floor(num(bootstrap.completedChunks,0))),
      totalChunks: Math.max(0,Math.floor(num(bootstrap.totalChunks,0))),
    },
...
function normalizeRapportClocks(value: unknown): StoryState['continuity']['rapportClocks'] {
  ...
  for(const [name,raw] of Object.entries(object(value))){ const c=object(raw); out[name]={rapport:clampInt(c.rapport,0,5,0),lastInteractionAt:Math.max(0,num(c.lastInteractionAt,0)),lastMeaningfulAt:Math.max(0,num(c.lastMeaningfulAt,0)),cooldownUntil:Math.max(0,num(c.cooldownUntil,0)),partnerMeaningfulUntil:Math.max(0,num(c.partnerMeaningfulUntil,0))}; }
  return out;
}
function normalizeSlowBondEvidence(value:unknown):NpcTrackerEntry['slowBondEvidence']{
  const src=object(value),counts=object(src.counts),out=blankSlowBondCounts();
  for(const k of Object.keys(out) as Array<keyof typeof out>)out[k]=clampInt(counts[k],0,20,0);
...
function normalizeCurrencyEntries(value:unknown):PlayerSheet['currency']{
  if(!Array.isArray(value))return[];
  const merged=new Map<string,{currency:string;amount:number}>();
  for(const raw of value){const o=object(raw),currency=text(o.currency).slice(0,60),amount=num(o.amount,0);if(!currency||!Number.isFinite(amount))continue;const key=currency.toLowerCase();const prev=merged.get(key);merged.set(key,{currency:prev?.currency||currency,amount:(prev?.amount||0)+amount});}
