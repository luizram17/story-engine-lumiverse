...
export function combatOutcome(margin:number, actionLength:number) {
  let outcome:{ outcomeTier:OutcomeTier; landedActions:number; outcome:string; counterPotential:'none'|'light'|'medium'|'severe' };
...
export function nonHostileOutcome(margin:number) {
  if(margin>=1) return {outcomeTier:'Success' as OutcomeTier,landedActions:1,outcome:'success',counterPotential:'none' as const};
  if(margin===0) return {outcomeTier:'Stalemate' as OutcomeTier,landedActions:0,outcome:'struggle',counterPotential:'none' as const};
  return {outcomeTier:'Failure' as OutcomeTier,landedActions:0,outcome:'failure',counterPotential:'none' as const};
}

export function resolveTurn(state:StoryState, semantic:SemanticLedger, fingerprint:string, seed:string, settings:{randomEvents:boolean;randomEventChance:number;proactivity:boolean;nameStyle:string}):TurnResolution {
  const rng=new TurnRng(seed);
  const actions=semantic.actions.slice(0,MAX_ACTIONS).map(a=>({...a}));
  const rolls:RollResult[]=[];
  const healthEvents:TurnResolution['healthEvents']=[];
  ...
  if(bGate.mode==='grace') refereeNotes.push(`Boundary grace: ${bGate.target} gets a warning before this access/restraint pressure becomes an opposed roll. Do not narrate compliance or successful control.`);
  else if(bGate.mode==='force') refereeNotes.push(`Boundary contest: ${bGate.target}'s boundary is immediately opposed or its warning threshold was reached; resolve relevant pressure as a contested action.`);
  ...
  for(let i=0;i<actions.length;i++){
    ...
    if(targetsBoundary&&bGate.mode==='grace'){ continue; }
    if(targetsBoundary&&bGate.mode==='force'&&!action.rollNeeded){action.rollNeeded=true;action.challengeType=action.kind==='restraint'?'restraint':'environment';action.stat=action.stat==='NONE'?'PHY':action.stat;}
    ...
    if(!action.rollNeeded || action.challengeType==='none') continue;
  ...
  for(const a of aggression) if(a.damage>0) healthEvents.push({targetType:'user',target:'',kind:'damage',amount:a.damage});
  const randomEvent:RandomEventResult=resolveRandomEvent(state,semantic,rng,settings.randomEvents,settings.randomEventChance);
...
function validateAvailability(state:StoryState,action:SemanticAction):{ok:boolean;note:string}{
  const player=state.player;
  if(!player)return {ok:true,note:''};
  if(action.itemUse){
    ...
    if(!owned)return {ok:false,note:`Unavailable item: ${action.itemUse}. Do not narrate it as owned or successfully used.`};
  }
  if(action.abilityUse){
    ...
    if(!known)return {ok:false,note:`Unavailable ability/spell/natural weapon: ${action.abilityUse}. Do not grant the attempted capability.`};
  }
...
function resolveAction(state:StoryState,action:SemanticAction,index:number,rng:TurnRng,seed:string):RollResult {
  ...
  if(action.target && action.challengeType!=='environment'){
    ...
    if(health) oppositionStat+=Math.max(-8,impairmentForCondition(conditionFromActor(health)));
  } else {
...
function buildHealthEvents(action:SemanticAction, result:RollResult, events:TurnResolution['healthEvents']):void {
  ...
  if(dmg<=0) return;
  if(action.target) events.push({targetType:'npc',target:action.target,kind:'damage',amount:dmg,nonlethal:action.harmMode==='nonlethal'});
}

export function resolveProactivity(state:StoryState,sem:SemanticLedger,rolls:RollResult[],rng:TurnRng):ProactivityResult[]{
  ...
  for(const actor of sem.actors){
    ...
    const tier:ProactivityResult['tier']=forced?'FORCED':crisis?'HIGH':npc.companion?'MEDIUM':Math.abs(relation)>=3?'MEDIUM':'LOW';
...
export function resolveNpcAggression(state:StoryState,sem:SemanticLedger,rolls:RollResult[],proactivity:ProactivityResult[],rng:TurnRng):AggressionResult[]{
  const results:AggressionResult[]=[];
  const defense=resolvePlayerEquipmentDefense(state);
  for(const p of proactivity){
    if(!p.proactive || p.intent!=='ESCALATE_VIOLENCE') continue;
    const npc=state.npcs[p.npc]; if(!npc||npc.status!=='active')continue;
    const failedAgainst=rolls.find(r=>r.target===p.npc&&r.counterPotential!=='none');
    const source:AggressionResult['source']=failedAgainst?'forced_counter':'proactivity';
    const counterBonus=failedAgainst?({light:2,medium:4,severe:6,none:0} as const)[failedAgainst.counterPotential]:0;
...
function resolveLootSearch(state:StoryState,semantic:SemanticLedger,seed:string,notes:string[]):TurnResolution['lootResult'] {
  const loot=semantic.loot; if(!loot?.present)return null;
  if(!['humanoid','monster'].includes(loot.targetKind)){ notes.push(`Loot search for ${loot.target||'target'} is not a tracked corpse/body; deterministic corpse loot was not generated.`); return {target:loot.target,status:'not_applicable'}; }
  const npc=state.npcs[loot.target];
  if(!npc){ notes.push(`Loot search rejected: ${loot.target||'target'} is not a tracked NPC/body.`); return {target:loot.target,status:'unknown_target'}; }
  const health=state.health.npcs[npc.name];
  if(!health?.dead){ notes.push(`Loot search rejected: ${npc.name} is not verified dead in hidden health state.`); return {target:npc.name,status:'target_not_dead'}; }
  if(npc.lootSearchCompleted){ notes.push(`Loot search already completed for ${npc.name}; do not generate a second deterministic loot envelope.`); return {target:npc.name,status:'already_searched'}; }
...
function calculateXp(rolls:RollResult[]):number{
  let xp=0;
  for(const r of rolls){ if(r.outcomeTier==='Critical_Success') xp+=30; else if(r.outcomeTier==='Moderate_Success') xp+=20; else if(['Minor_Success','Success'].includes(r.outcomeTier)) xp+=10; }
