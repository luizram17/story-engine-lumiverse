import type { NpcTrackerEntry, OutcomeTier, SemanticLedger, StoryState, RollResult } from '../shared/types.js';
...
export function updateRelationships(state: StoryState, semantic: SemanticLedger, rolls: RollResult[], seed: string): string[] {
  ...
  for (const actor of semantic.actors) {
    ...
    if(!existed){
      ...
      if(!explicitInitial){
        if(actor.relation==='benefited'){npc.bond=1; change=`positive initial impression for ${npc.name}`;}
        else if(actor.relation==='opposed'){npc.hostility=1; change=`adverse initial impression for ${npc.name}`;}
        else if(actor.relation==='harmed'){npc.hostility=2;npc.fear=1;change=`hostile initial impression for ${npc.name}`;}
      ...
      if(actor.personalitySummary&&!npc.personalitySummary)npc.personalitySummary=actor.personalitySummary;
      if(actor.initialRelationshipDescriptors?.length)npc.relationshipDescriptors=mergeDescriptors(npc.relationshipDescriptors,actor.initialRelationshipDescriptors);
    ...
    if(semantic.intimacyAdvanceExplicit && actor.relation==='direct'){
      if(npc.boundary?.active || npc.hostility>=2 || npc.fear>=3){
        npc.hostility+=1; change=`intimacy advance crossed an active boundary for ${npc.name}`;
      }else if(npc.bond>=3 && positive){
        npc.intimacy+=strong?2:1;
        if(npc.intimacy>=3)npc.romanceStage='dating'; else if(npc.intimacy>=1)npc.romanceStage='interest';
...
export function standingInfluence(state: StoryState, npcName: string): number {
